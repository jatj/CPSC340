...
if __name__ == "__main__":
    ...
    if question == "3.4":
        ...
        stats_rows = ["minimum", "maximum", "mean", "median", "mode"]
        stats = pd.DataFrame(index=stats_rows, columns=names, data = [
            df.min().values,
            df.max().values,
            df.mean().values,
            df.median().values,
        ...
        quatiles = pd.DataFrame(index=quatiles_rows, data=[
            df.quantile(0.05),
            df.quantile(0.25),
            df.quantile(0.5),
            df.quantile(0.75),
        ...
        # ----------------------------------- 5.1.3
        hmean = stats.loc["mean"].idxmax()
        lmean = stats.loc["mean"].idxmin()
        hvar = df.var().idxmax()
        lvar = df.var().idxmin()
        regions_rows = ["Highest","Lowest"]
        regions_cols = ["Mean","Variance"]
        regions = pd.DataFrame(index=regions_rows, columns=regions_cols, data=[
            [hmean, hvar],
            [lmean, lvar]
        ])
        print("\n************************************************ 5.1.3 REGIONS \n")
